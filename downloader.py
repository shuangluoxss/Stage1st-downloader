...
def login(username, password, on_success, on_fail):
    ...
    resp = sess.post(
        "https://stage1st.com/2b/member.php?mod=logging&action=login&loginsubmit=yes&infloat=yes&lssubmit=yes&inajax=1",
        data={"username": username, "password": password},
    ).text
    if "https://stage1st.com/2b/./" in resp:
        sess.headers.update(
            {
                "Host": "stage1st.com",
...
class S1Downloader(object):
    ...
    def download_source(self, source):
        ...
        if not re.match("http", source):
            ...
            # 辨析文件名
            source_filename = source_root + re.sub(
                r'[\\:*?"<>|]', "_", source.split("//")[1]
            )
            logger.debug(
                "downloader source = %s, filename = %s", source, source_filename
            )
            ...
            if not os.path.exists("data/" + source_filename):
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        logger.warning(
                            "File failed to download. Source = %s, HTTP status code = %d",
                            source,
                            resp.status_code,
                        )
                        return None
        ...
        for j in range(5):
            try:
                if not soup:
                    time.sleep(2)
                    page = self.sess.get(
                        "https://stage1st.com/2b/" + filename, timeout=30
                    ).text
                ...
                script_list = list(
                    filter(
                        lambda x: x.get("src")
                        and "google" not in x.get("src")
                        and "home.php" not in x.get("src"),
                        soup.find_all("script"),
                    )
                )
                css_list = list(
                    filter(
                        lambda x: x.get("rel") == ["stylesheet"], soup.find_all("link")
                    )
                )
                for img in img_list:
                    if (
                        img.get("src")
                        == "source/static.saraba1st.com/image/common/none.gif"
                    ):
                        img["src"] = img.get(
                            "file", "source/static.saraba1st.com/image/common/none.gif"
                        )
                # 删除.css后的多余部分
                for x in css_list:
                    if x.get("href"):
    ...
    def download_thread(self, url, force_refresh=False, pn_range=None):
        """
        url为待下载专楼地址，支持'https://stage1st.com/2b/thread-{tid}-{page}-1.html'与
                               'https://stage1st.com/2b/forum.php?mod=viewthread&tid={tid}&page={page}'两种格式
        ...
        for j in range(3):
            try:
                ...
                soup = BeautifulSoup(page, "html.parser")
                total_page = int(
                    (re.findall(r'<span title="共 (\d+) 页">', page) + [1])[0]
                )
                title = (
        ...
        if title == "":
            logger.error(
                "Download failed. Please check Internet connection or authorization."
            )
        ...
        with open(
            "%s.html" % re.sub(r'[\\/:*?"<>|]', "_", title), "w+", encoding="utf-8"
        ) as file:
            file.write(
                '<head><meta http-equiv="refresh" content="0;url=data/%s"></head>'
                % (url_template % pn_range[0])
            )
            # 更新进度
